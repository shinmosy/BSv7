...
○ ${usedPrefix + command} lock 5menit
        ...
        console.error(error);
        await conn.reply(m.chat, 'Terjadi kesalahan dalam eksekusi perintah.', m);
    }
};

handler.help = ['group *open / close / unlock / lock* *waktu*'];
handler.tags = ['group'];
handler.command = /^(group|gcsetting|groupset)$/i;
handler.admin = true;
