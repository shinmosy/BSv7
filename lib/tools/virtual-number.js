...
export class smstome {
  async Country() {
    try {
      const { data } = await axios.get('https://smstome.com');
      ...
      return $('.column.fields ul li').map((_, listItem) => {
        ...
        return { title, countryCode, countryFlag, link };
  ...
  async getNumber(country) {
    try {
      const { data } = await axios.get(`https://smstome.com/country/${country.toLowerCase()}`);
      const $ = cheerio.load(data);
      return $('.numview').map((_, numview) => {
        ...
        return { phoneNumber, location, addedDate, link };
      }).get().filter(entry => Object.values(entry).every(value => value !== undefined && value !== ''));
    } catch (error) {
  ...
  async getMessage(url, page) {
    try {
      const { data } = await axios.get(page ? `${url}?page=${page}` : url);
      const $ = cheerio.load(data);
      return $('table.messagesTable tbody tr').map((_, message) => {
        const from = $('td:nth-child(1)', message).text().trim().replace('<!--sse-->', '').replace('<!--/sse-->', '');
        const received = $('td:nth-child(2)', message).text().trim().replace('<!--sse-->', '').replace('<!--/sse-->', '');
        const content = $('td:nth-child(3)', message).text().trim().replace('<!--sse-->', '').replace('<!--/sse-->', '');
        return { from, received, content };
      }).get().filter(entry => Object.values(entry).every(value => value !== undefined && value !== ''));
    } catch (error) {
...
export class sms24 {
  ...
  async getNumber(country) {
    try {
      const { data } = await axios.get(`https://sms24.me/en/countries/${country.toLowerCase()}`);
      const $ = cheerio.load(data);
      return $('.callout').map((_, callout) => {
  ...
  async getMessage(number) {
    try {
      const { data } = await axios.get(`https://sms24.me/en/numbers/${parseInt(number)}`);
      const $ = cheerio.load(data);
