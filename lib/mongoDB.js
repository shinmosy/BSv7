...
export class mongoDB {
  constructor(url, options = defaultOptions) {
    this.url = url || 'mongodb+srv://Wudysoft:Wudysoft@wudysoft.2hm26ic.mongodb.net/Taylor-V2?retryWrites=true&w=majority&appName=Wudysoft';
  ...
  async read() {
    try {
      const schema = new Schema({ data: { type: Object, required: true, default: {} } });
      const _model = await this.db.model('data', schema);
      const _data = await _model.findOne({});
      this.data = (_data?.data) ? _data.data : {};
      const [_ignored, newData] = await Promise.all([this.write(this.data), _model.findOne({})]);
      this._data = newData;
...
export class mongoDBV2 {
  ...
  async read() {
    try {
      ...
      const list = await this.db.model('lists', schema);
      const listsData = await list.findOne({});
      if (!listsData?.data) {
        await list.create({ data: [] });
        this.lists = await list.findOne({});
      }
      const garbage = [];
      for (const { name } of listsData?.data || []) {
        let collection;
        try {
        ...
        if (collection) {
          this.models.push({ name: collectionName, model: collection });
          const collectionsData = await collection.find({});
          this.data[collectionName] = Object.fromEntries(collectionsData.map(v => v.data));
        }
      }
      const updatedList = await list.findOneAndUpdate({ _id: this.lists?._id }, { data: listsData.data.filter(v => !garbage.includes(v.name)) }, { new: true });
