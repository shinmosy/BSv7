...
const formatTime = (timestamp) => {
    try {
        ...
        const formattedDate = new Date(durationInSeconds).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
        const formattedDateString = new Date(durationInSeconds).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' });
...
class ClearLogger {
    logCount = 0;
    addLog = () => ((this.logCount >= 99) && this.clearLogs(), this.logCount++);
    clearLogs = () => (console.log(chalk.blue("Clearing console...")) && console.clear(), this.logCount = 0);
}
...
clearLoggerInstance.addLog();
    ...
    if (m.sender !== conn.user.jid) {
        if (["BAE5", "WA"].some(k => idBot.includes(k))) {
            ...
            } else if (!["BAE5", "WA"].some(k => idBot.includes(k))) {
                conn.user.listbot[chatKey] = conn.user.listbot[chatKey].filter(bot => bot.number !== m.sender);
                conn.logger.info('Removed bot number ' + m.sender.split('@')[0]);
            }
        }
    }
}
