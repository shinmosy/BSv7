...
if (m?.isGroup && m?.sender) {
    const idBot = m.msg?.id || m.key.id || 'N/A';
    if (["BAE5", "WA"].some(k => idBot.includes(k) && m.sender !== conn.user.jid)) {
        conn.user.listbot = conn.user.listbot || new Set();
        const isNewBot = !conn.user.listbot.has(m.sender);

        if (isNewBot) {
            conn.user.listbot.add(m.sender);
            conn.logger.info('New bot number ' + m.sender.split('@')[0]);
        } else {
            conn.user.listbot.delete(m.sender);
