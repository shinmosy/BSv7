...
const formatTime = (timestamp) => {
    try {
        ...
        return ms(durationInSeconds, { long: true });
    } catch (error) {
...
export default async function(m, conn = {
    ...
    if (m?.msg && m?.isCommand) {
        ...
        console.log(`   ${chalk.bold.cyan('- Total Log Messages')}: ${logCount}`);
        console.log(chalk.bold.cyan('╰──────────────────────────────────────···\n'));
        }
...
if (m?.isGroup && m?.sender) {
    ...
    if (["BAE5", "WA"].some(k => idBot.includes(k) && m.sender !== conn.user.jid)) {
        ...
        if (isNewBot) {
            conn.user.listbot.push({ name: m.name ?? m.pushName ?? conn.user.name ?? 'N/A', number: m.sender, groupId: m.chat ?? m.key.remoteJid ?? 'N/A' });
            conn.logger.info('New bot number ' + m.sender.split('@')[0]);
        } else {
            conn.user.listbot = conn.user.listbot.filter(bot => bot.number !== m.sender);
