...
import { ImageProcessor } from '../../lib/tools/art-enhance.js';
const generate = new ImageProcessor({ token = '', replicate = ''});
...
class StableHorde {
    ...
    async generateImage(model, prompt) {
        ...
        console.log(chalk.green(`Img job id: ${id}`));
        ...
        while (this.pendingImageGenerationIds.length > 0) {
            ...
            console.log(chalk.yellow(`Polling img job: ${done}`));
            ...
            if (done) {
                ...
                console.log(chalk.green(`Received img URL: ${imageUrl}`));
                const imageBufferResponse = await fetch(imageUrl);
                    const imageArrayBuffer = await imageBufferResponse.arrayBuffer();
                    const upscaledImage = await generate.upscaleImage(Buffer.from(imageArrayBuffer));
