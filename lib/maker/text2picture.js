...
async function toSticker(inp) {
    try {
        const phrase = inp.map((result, index) => `${result}${index !== 0 && (index + 1) % 3 === 0 ? "%0A" : "%20"}`).join("");
        const APIURL = new URL("https://raterian.sirv.com/New%20Project.png");
        
        APIURL.searchParams.set("text.0.text", phrase);
        APIURL.searchParams.set("text.0.position.y", "-50%25");
        APIURL.searchParams.set("text.0.color", "000000");
        APIURL.searchParams.set("text.0.font.family", "Poppins");
        APIURL.searchParams.set("text.0.font.weight", "600");
        APIURL.searchParams.set("text.0.outline.color", "ffffff");
        APIURL.searchParams.set("text.0.outline.width", "5");
