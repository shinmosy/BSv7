...
export default class APIManager {
    ...
    async fetchCityWeather(cityName, unit) {
        try {
            const { data } = await axios.get(`${this.#openWeatherMapBaseUrl}weather`, {
                params: {
                    q: cityName,
    ...
    async fetchIp() {
        try {
            const { data } = await axios.get(`${this.#ipInfoBaseUrl}/json`, {
                params: {
    ...
    async fetchImage(city) {
        try {
            const { data } = await axios.get(`${this.#unsplashBaseUrl}/random`, {
                params: {
                    query: city,
                    count: 1,
    ...
    async getSocialSearcherPosts({ q, networks }, { limit = 80, page, requestid } = {}) {
        if (limit < 0 || limit > 100) return undefined;

        try {
            const { data } = await axios.get('https://api.social-searcher.com/v2/search', {
                params: {
                    q: q.join('OR'),
                    lang: 'pt-BR',
    ...
    async getSocialSearcherUserPosts({ userId, networks }, { limit = 20, page, requestid }) {
        if (limit < 0 || limit > 100) return [];

        try {
            const { data } = await axios.get(`https://api.social-searcher.com/v2/users/${userId}/posts`, {
                params: {
                    lang: 'pt-BR',
                    network: networks.join(','),
                    limit,
    ...
    async searchSocialSearcherUsers(accountName, networks) {
        try {
            const { data } = await axios.get('https://api.social-searcher.com/v2/users', {
                params: {
                    q: accountName,
                    network: networks.join(','),
                },
    ...
    async FBGET(leiam) {
        try {
            const { data } = await axios.post('https://www.thetechlearn.com/video-downloader/wp-json/aio-dl/video-data/', { 
                url: leiam 
            });
    ...
    async IMGUR(leiam) {
        try {
            const { data } = await axios.post("https://api.imgur.com/3/image", leiam, {
                headers: {
