            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                */
                ...
                let message = {
                    ...
                    caption: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text,
                    ptt,
                    [mtype]: {
                        url: pathFile
                    },
        ...
        */
        sendListM: {
            ...
            async value(jid, button, rows, quoted, options = {}) {
                ...
                return await conn.sendMessage(jid, {
                    ...
                    text: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text
                }, {
                    quoted,
                    ...options
                })
        ...
        sendPayment: {
            async value(jid, amount, currency, text = '', from, image, options) {
                ...
                const requestPaymentMessage = {
                    ...
                    noteMessage: {
                        extendedTextMessage: {
                            text: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text || 'Example Payment Message'
                        }
                    },
        ...
        sendPoll: {
            async value(jid, text = '', optiPoll, options) {
                ...
                if (!options) options = {}
                const pollMessage = {
                    name: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text,
        ...
        sendButton: {
            ...
            async value(jid, text = '', footer, image, button, quoted, options) {
                return !image ? await conn.sendMessage(jid, {
                    ...
                    quoted,
                    ...options
                }) : conn.sendFile(jid, image, author, conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + button.map(([title, command]) => ({
                        title,
                        command
                    })).map((v, index) => {
                        return `*[ ${index + 1} ] Name:* ${v.title || ''}\n➥ *CMD:* ${v.command || ''}`.trim()
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', b, c = '', d = '', e = '', f = '', buttons, quoted, options) {
                ...
                return !b ? await conn.sendMessage(jid, {
                    ...
                    quoted,
                    ...options
                }) : conn.sendFile(jid, b, footer, conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + c + '\n\n' + d + '\n\n' + e + '\n\n' + f + '\n\n' + button.map(([title, command]) => ({
                        title,
                        command
                    })).map((v, index) => {
                        return `*[ ${index + 1} ] Name:* ${v.title || ''}\n➥ *CMD:* ${v.command || ''}`.trim()
        ...
        */
        sendHydrated2: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
                ...
                let message = {
                    ...options,
                    [buffer ? 'caption' : 'text']: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyText(text, parseInt(conn.temafont)) ?? FancyTextV2(text, parseInt(conn.temafont)) : text || '',
                    footer,
                    templateButtons,
                    ...(buffer ?
                        options.asLocation && /image/.test(type.mime) ? {
