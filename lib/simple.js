        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                */
                ...
                let message = {
                    ...Object.assign({ mentions: typeof text === 'string' ? await conn.parseMention(text || '@0') : [], contextInfo: {
...
END:VCARD`.trim()
  ...
  : text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + button.map(([title, command]) => ({
                    ...
                    quoted,
                    ...options
                }) : conn.sendFile(jid, image, author, (conn.temafont && !isNaN(parseInt(conn.temafont)))
  ? await generateFont(text, parseInt(conn.temafont))
  : text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + button.map(([title, command]) => ({
                        title,
                        command
