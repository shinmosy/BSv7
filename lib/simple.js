...
import storeSystem from './store-multi.js';
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                */
                ...
                let message = {
                    ...Object.assign({ mentions: await conn.parseMention(text || '@0'), contextInfo: { mentionedJid: await conn.parseMention(text || '@0') } }, { ...(options || {}), ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) }),
                    caption: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text,
                    ptt,
                    [mtype]: {
                        url: pathFile
                    },
        ...
        */
        sendListM: {
            ...
            async value(jid, button, rows, quoted, options = {}) {
                ...
                return await conn.sendMessage(jid, {
                    ...Object.assign({ mentions: await conn.parseMention(text || '@0'), contextInfo: { mentionedJid: await conn.parseMention(text || '@0') } }, { ...(options || {}), ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) }),
                    text: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text
                }, {
                    quoted,
                    ...options
                })
        ...
        sendPayment: {
            async value(jid, amount, currency, text = '', from, image, options) {
                ...
                const requestPaymentMessage = {
                    ...
                    noteMessage: {
                        extendedTextMessage: {
                            text: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text || 'Example Payment Message'
                        }
                    },
        ...
        sendPoll: {
            async value(jid, text = '', optiPoll, options) {
                ...
                if (!options) options = {}
                const pollMessage = {
                    name: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text,
    ...
    */
        ...
        */
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options = {}) {
                ...
                const data = isBuffer ?
                    ...
                    Buffer.from(PATH.split(",")[1], 'base64') :
                    (/^https?:\/\//.test(PATH) && await isTextContent(PATH)) ?
                    await (await getDataBuffer(PATH)) :
                    fs.existsSync(PATH) ?
                    ((filename = PATH), fs.readFileSync(PATH)) :
                    undefined;
                return data ?
        ...
        sendButton: {
             ...
             * @param {Object} options
             */
            async value(jid, text = '', footer, image, button, quoted, options) {
                return !image ? await conn.sendMessage(jid, {
                    ...Object.assign({ mentions: await conn.parseMention(text || '@0'), contextInfo: { mentionedJid: await conn.parseMention(text || '@0') } }, { ...(options || {}), ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) }),
                    text: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + button.map(([title, command]) => ({
                            title,
                            command
                    ...
                    quoted,
                    ...options
                }) : conn.sendFile(jid, image, author, conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + button.map(([title, command]) => ({
                        title,
                        command
                    })).map((v, index) => {
                        return `*[ ${index + 1} ] Name:* ${v.title || ''}\n➥ *CMD:* ${v.command || ''}`.trim()
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', b, c = '', d = '', e = '', f = '', buttons, quoted, options) {
                ...
                return !b ? await conn.sendMessage(jid, {
                    ...
                    quoted,
                    ...options
                }) : conn.sendFile(jid, b, footer, conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text + '\n\n' + readMore + '\n\n' + footer + '\n\n' + c + '\n\n' + d + '\n\n' + e + '\n\n' + f + '\n\n' + button.map(([title, command]) => ({
                        title,
                        command
                    })).map((v, index) => {
                        return `*[ ${index + 1} ] Name:* ${v.title || ''}\n➥ *CMD:* ${v.command || ''}`.trim()
        ...
        */
        sendHydrated2: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
                ...
                let message = {
                    ...options,
                    [buffer ? 'caption' : 'text']: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await FancyTextV2(text, parseInt(conn.temafont)) : await FancyText(text, parseInt(conn.temafont)) ?? text || '',
                    footer,
                    templateButtons,
                    ...(buffer ?
                        options.asLocation && /image/.test(type.mime) ? {
        ...
        sendStimg: {
            async value(jid, path, quoted, options = {}) {
                let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await getDataBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
                let buffer
        ...
        sendStvid: {
            async value(jid, path, quoted, options = {}) {
                ...
                let buffer
                if (options && (options.packname || options.author)) {
                    buffer = await Exif.writeExifVid(buff, options)
                } else {
                    buffer = await Exif.videoToWebp(buff)
                }
                await conn.sendMessage(jid, {
                    sticker: {
                        url: buffer
...
async function isTextContent(url) {
    try {
